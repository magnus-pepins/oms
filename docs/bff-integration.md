...
- `GET /api/internal/oms/v1/orders/{orderId}` — `customer-frontend/app/api/internal/oms/v1/orders/[orderId]+api.ts` (authenticated; forwards to OMS `GET /internal/v1/orders/{id}`; returns **404** `{"error":"not_found"}` if the order’s `accountId` does not match the session user, same as missing order; OMS **404** bodies are not passed through).
