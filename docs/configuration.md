...
| `SPRING_PROFILES_ACTIVE` | (unset) | Activate one of `oms-cluster-node`, `oms-postgres-projector`, `oms-fix-egress`, or `oms-ingress-replica` (mutually exclusive — see `TopologyWorkerProfiles`). Topology: [oms-aeron-cluster-substrate.md](../../system-documentation/plans/oms-aeron-cluster-substrate.md). |
