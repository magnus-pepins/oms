...
| `SPRING_PROFILES_ACTIVE` | (unset) | Include **`oms-control-worker`** for P3 prep: no new-order HTTP/gRPC ingress (`OmsProfiles`). Topology: [oms-ingress-control-fix-topology.md](../../system-documentation/plans/oms-ingress-control-fix-topology.md). |
...
Phase 3 slice 3f of [`oms-aeron-cluster-substrate`](../../system-documentation/plans/oms-aeron-cluster-substrate.md) deleted **`control_outbox`** + **`OutboxReconciler`**. The cluster's events recording is now the durable handoff between admission and downstream consumers (Postgres projector, FIX egress); there is no Postgres → Chronicle drain path any more. **`OMS_OUTBOX_RECONCILER_*`** environment variables and the **`oms_control_outbox_appended_total`** / **`oms.pipeline.control.outbox_to_chronicle_lag`** metrics are gone with it. Domain / ledger outboxes remain on their own reconcilers (sections below).
