...
- **HTTP ingress → FIX round-trip IT:** `FixIngressRoundTripSpringIntegrationTest` — same profile; **`POST /internal/v1/orders`** → outbox reconciler → **`ControlTailer.apply`** → **`WORKING`** → NOS → synthetic fill → **FILLED**.
