...
/**
 * Fail-fast rules for {@value OmsProfiles#VENUE_EGRESS} (Phase 3 of
 * {@code system-documentation/plans/oms-aeron-cluster-substrate.md}).
 *
 * <p>The FIX-egress JVM owns gRPC venue/J's {@code SocketInitiator} (broker constraint: exactly one per route) and
 ...
 * the {@code ORDER_ACCEPT_PROFILE} expression already excludes {@code oms-venue-egress}, but we double-check the
 * runtime invariants here so a bad config produces a startup failure with a clear message instead of a half-loaded
 * context.
 *
 * <p>Unlike the projector validator, this role <strong>does</strong> own gRPC venue: {@code oms.routing.backend=internal-venue}
 * and {@code oms.routing.backend=true} are both expected. We reject {@code oms.grpc.enabled=true} (no order
 * ingress on this JVM).
 *
 * <p><strong>Slice 3d:</strong> {@code oms.cluster.client.enabled=true} is now <em>required</em>: the egress
 * JVM offers {@link com.balh.oms.cluster.ApplyExecutionReportCommand} back to the cluster on inbound venue gRPC
...
@Component
@Profile("!test")
public class VenueEgressTopologyValidator {

    private final Environment environment;
    private final OmsConfig omsConfig;

    public VenueEgressTopologyValidator(Environment environment, OmsConfig omsConfig) {
    ...
    @PostConstruct
    void validate() {
        validateVenueEgressTopology(environment, omsConfig);
    }

    public static void validateVenueEgressTopology(Environment environment, OmsConfig omsConfig) {
        TopologyWorkerProfiles.validateNoConflictingWorkerProfiles(environment);
        if (!environment.acceptsProfiles(Profiles.of(OmsProfiles.VENUE_EGRESS))) {
            return;
        }
        if (omsConfig.getGrpc().isEnabled()) {
            throw new IllegalStateException(
                    "Spring profile "
                            + OmsProfiles.VENUE_EGRESS
