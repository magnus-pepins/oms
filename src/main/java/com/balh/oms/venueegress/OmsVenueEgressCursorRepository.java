...
/**
 * Persistence for the Aeron-log replay cursor consumed by {@link OmsVenueEgressService}.
 *
 * <p>Mirrors {@link com.balh.oms.projector.AeronProjectorCursorRepository} shape-for-shape on a
 * separate physical table ({@code oms_venue_egress_cursor}, V26 + V56). The egress JVM advances
 ...
 * pre-V56 test fixtures but are <b>no longer used</b> by {@link OmsVenueEgressService} on the
...
public class OmsVenueEgressCursorRepository {
    ...
    private static final String UPSERT_SQL = """
            INSERT INTO oms_venue_egress_cursor (egress_id, stream_id, last_applied_position, last_applied_at)
            ...
            ON CONFLICT (egress_id, stream_id) DO UPDATE
             ...
             WHERE oms_venue_egress_cursor.last_applied_position < EXCLUDED.last_applied_position
            """;
     ...
     * affordance that {@link OmsVenueEgressService#init} uses to detect rewinds.
     */
    private static final String UPSERT_WITH_RECORDING_SQL = """
            INSERT INTO oms_venue_egress_cursor
                (egress_id, stream_id,
            ...
            ON CONFLICT (egress_id, stream_id) DO UPDATE
               SET last_applied_recording_id = EXCLUDED.last_applied_recording_id,
                   ...
                   last_applied_at = EXCLUDED.last_applied_at
             WHERE oms_venue_egress_cursor.last_applied_recording_id IS NOT NULL
               AND (
                     EXCLUDED.last_applied_recording_id > oms_venue_egress_cursor.last_applied_recording_id
                  OR (
                       EXCLUDED.last_applied_recording_id = oms_venue_egress_cursor.last_applied_recording_id
                       AND EXCLUDED.last_applied_position > oms_venue_egress_cursor.last_applied_position
                     )
                   )
            """;
     ...
     * {@link OmsVenueEgressService#init} can detect the rewind and refuse to start without
    ...
    private static final String SET_HIGH_WATER_SQL = """
            UPDATE oms_venue_egress_cursor
    ...
    public OmsVenueEgressCursorRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
     ...
     * {@link OmsVenueEgressService#init} to enforce the rewind guard: {@code last_applied}
     ...
     * leaves it {@code NULL} on a fresh insert. Production code in {@link OmsVenueEgressService}
     ...
     * loud-fail path in {@link OmsVenueEgressService#init} refuse to start. Use
