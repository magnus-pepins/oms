...
/**
 * Same as {@link FixIngressRoundTripSpringIntegrationTest} with {@code oms.fix.outbound-driver=dedicated}
 * to exercise the dedicated outbound thread + {@link com.balh.oms.fix.FixRouteDispatcher#pollPending} path.
 */
@Import(FixRoundTripTestBeans.class)
@ActiveProfiles({"test", "fix-roundtrip-it"})
class FixIngressRoundTripDedicatedOutboundSpringIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    void httpPostThenControlApply_sendsNosAndAcceptorFill_reachesFilled() throws Exception {
        ...
        UUID orderId = postOrder(accountId, "fix-http-rt-dedicated-1");
