...
/**
 * Same as {@link FixIngressRoundTripSpringIntegrationTest} with {@code oms.fix.outbound-handoff-transport=postgres}
 * (durable handoff table + {@code SKIP LOCKED} pop).
 */
@Import(FixRoundTripTestBeans.class)
@ActiveProfiles({"test", "fix-roundtrip-it"})
class FixIngressRoundTripPostgresHandoffSpringIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    static void fixRoundTripProps(DynamicPropertyRegistry registry) {
        ...
        registry.add("oms.fix.outbound-handoff-transport", () -> "postgres");
    ...
    void httpPostThenControlApply_sendsNosAndAcceptorFill_reachesFilled() throws Exception {
        ...
        UUID orderId = postOrder(accountId, "fix-http-rt-pghandoff-1");
